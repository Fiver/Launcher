...
An "alpha" version is in the downloads, I am not done coding things I want into this. The generic launcher will be coded when I am done with this one's features.

Basically it's missing the ability to torrent files, compare MD5s (in but not implemented), repack patch Mopaq files to master Mopaq files (in but not implemented), and download zips from github repos and extract them to specific directories.

There's also a slew of features I haven't gotten to that are in the interface but not handled
...
The prefix path I used was "C:\\boost" if you use another prefix path you will have to change include/lib directories.
